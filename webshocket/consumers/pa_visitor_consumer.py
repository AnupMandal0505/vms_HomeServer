...
class AppointmentConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        self.pa_user_id = query_string.get("pa_user_id", [None])[0]  # Extract pa_user_id
        # print(" WebSocket Connected for Appointment Updates, User ID:", self.pa_user_id)
    ...
    async def send_appointment_update(self, event):
        ...
        data = event["data"]
        if data.get("created_by") == self.pa_user_id:  
